...
function filterCategories(data) {
  return data.filter((category) => !BAD_IDS.has(Number(category.id)));
}
...
// helper to select random, unique categories from the filtered list
...
export default {
  ...
  methods: {
    ...
    async handleQuestionClick(category, rowIndex) {
      ...
      try {
        ...
        if (!question) {
          console.log(
            "No question received from API -- may have exceeded rate limit/retries. Please try again later.",
          );
          return;
        }
        ...
        this.doubleJeopardy = Math.random() < 0.1;
        this.wager = null;
    ...
    async fetchQuestion(categoryId, difficulty, retries = 3, backoff = 2000) {
      try {
        console.log(
          `fetching questions with cat: ${categoryId} and diff: ${difficulty}`,
        );
        const response = await fetch(
          `https://opentdb.com/api.php?amount=1&category=${categoryId}&difficulty=${difficulty}&type=boolean`,
        );

        // trivia api limits calls to once per 5 seconds
        if (response.status === 429) {
          if (retries > 0) {
            // limit to 3 retries so we don't wait forever
            console.warn(
    ...
    answerQuestion(answer) {
      ...
      const amount =
        this.doubleJeopardy && this.wager ? this.wager : this.modalValue;
    ...
    calculateWinner() {
      let maxBalance = Math.max(
        ...this.players.map((player) => player.balance),
      );
      let winners = this.players.filter(
        (player) => player.balance === maxBalance,
      ); // could be multiple if there's a tie
  ...
  computed: {
    ...
    maxWager() {
      return Math.max(
        this.players[this.currentPlayerIndex].balance,
        this.modalValue,
      );
    },
  },
...
<template>
  ...
  <div v-if="setupActive" class="gameSetup">
    <label
      >Number of players:
      ...
      <template v-else>
        ...
        >
          ...
          <article
            ...
            @click="
              !usedQuestions[`${category.id}-${i - 1}`] &&
              handleQuestionClick(category, i - 1)
            "
          >
